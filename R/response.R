#' get_coefficients
#'
#' @description Get the coefficients with names related to a particular step_*
#'
...
response_harmonic <- function(co, step_info) {
  ...
  tibble(
    x = step_info[grep("sin", step_info$key), ]$frequency,
    sin_coefficient,
    cos_coefficient,
...
#'  `step_distributed_lag`, `step_lead_lag`, `step_harmonic` and
#'  `step_earthtide`.
#'
...
response.numeric <- function(fit, rec, verbose = FALSE, ...) {
  ...
  for (i in seq_len(nrow(rec_steps))) {
