...
#'   This is an efficient combination of `recipes::step_cut` and `recipes::step_dummy`.
#'
...
#'   'one_hot'.
#' @param prefix A prefix for generated column names, default to "find_interval_".
#'
...
step_find_interval_new <-
  function(terms, role, trained, vec, encoding, prefix, keep_original_cols, columns, skip, id) {
    step(
...
bake.step_find_interval <- function(object, new_data, ...) {
  ...
  for(i in seq_along(object$columns)) {
    ...
    if(n_vec <= 1) {
      ...
      interval_dummy <- to_dummy(interval_f, n_vec)
      colnames(interval_dummy) <- column_names
      new_data <- bind_cols(new_data, as_tibble(interval_dummy[, -1L, drop = FALSE]))
...
tidy2.step_find_interval <- function(x, ...) {
  ...
  res <-
    tibble(terms = rep(sel2char(x$terms), each = length(x$vec)),
           vec = rep(x$vec, times = n_terms)
    )
  res$key <- paste0(x$prefix, rep(x$vec, times = n_terms), '_', res$terms)
  res$id <- x$id
