...
#'  location. This step requires the
#'  [earthtide package](https://CRAN.R-project.org/package=earthtide).
#'
...
step_earthtide_new <-
  function(terms, role, trained, do_predict, method, astro_update, latitude,
           longitude, elevation, azimuth, gravity, earth_radius, earth_eccen,
           cutoff, wave_groups, catalog, eop, scale, prefix, columns,
...
bake.step_earthtide <- function(object, new_data, ...) {
  ...
  if (object$do_predict) {
    ...
    names(et) <- paste0(
      ...
      "_", rep(seq_len(nrow(object$wave_groups)), each = 2)
    )
  }
...
tidy2.step_earthtide <- function(x, ...) {
  ...
  res <-
    tibble(
      terms = rep(sel2char(x$terms), n_terms * 2),
      ...
      frequency = rep(earthtide::get_main_frequency(
        x$wave_groups$start,
        x$wave_groups$end
      ), times = 2)
    )
