...
tinytest::expect_equivalent(unlist(frec$get_step_data("barometric_efficiency")),
                            rep(0.3, 3), info = "StepBaroHarmonic works with no Earth tides")

Mod(frecipes:::be_transfer(qM(kennel_2020[, list(wl, baro, et)]), c(3),
...
Mod(frecipes:::be_transfer(qM(kennel_2020[, list(wl, baro, et)]), 3,
...
Mod(frecipes:::be_transfer(qM(dat[, list(wl, baro, et)]), c(3,3,3),
...
tmp2 <- sapply(1:200, function(x) {
  frecipes:::dft_goertzel(qM(kennel_2020[, list(wl)]), x)
})
tmp1 <- sapply(1:150, function(x) {
Mod(frecipes:::be_dft(qM(kennel_2020[, list(wl, baro, et)]), x))
})
plot((tmp1[1,]), type = 'l', log = 'y')

tmp1a <- sapply(1:150, function(x) {
  frecipes:::be_dft(qM(kennel_2020[1:20000, list(wl, baro, et)]), x)
})

k <- 1
test <- sapply(1:200, function(x) {
frecipes:::be_dft(qM(kennel_2020[(1440 * k):(1440*(8+k)), list(wl, baro, et)]), x)
})
...
tmp1b <- sapply(1:150, function(x) {
  frecipes:::be_dft(qM(kennel_2020[20000:40000, list(wl, baro, et)]), x)
})

tmp1c <- sapply(1:150, function(x) {
  frecipes:::be_dft(qM(kennel_2020[40000:60000, list(wl, baro, et)]), x)
})

tmp1d <- sapply(1:150, function(x) {
  frecipes:::be_dft(qM(kennel_2020[60000:80000, list(wl, baro, et)]), x)
})

plot(Mod(tmp1a[2,] + tmp1b[2,] + tmp1c[2,] + tmp1d[2,]), type = 'l')


tf <- frecipes:::transfer_pgram(qM(kennel_2020[, list(wl, baro, et)]), spans = c(3), FALSE, FALSE, 0.45)
...
tmp <- frecipes:::be_dft(qM(kennel_2020[, list(wl, baro, et)]), 113)
...
tinytest::expect_equivalent(unlist(frec$get_step_data("barometric_efficiency"))[2:3],
