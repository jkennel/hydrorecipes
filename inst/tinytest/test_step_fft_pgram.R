...
gain <- as.numeric(Mod(frecipes:::transfer_pgram(m,
                                      spans = 3,
                                      detrend = FALSE,
                                      demean = FALSE,
                                      taper = 0.1)))

tinytest::expect_equivalent(gain, rep(0.2, n),
                            info = "transfer_pgram gives the right gain")


n_groups <- 50
gain = as.numeric(Mod(frecipes:::transfer_pgram_smooth(m,
                                                       spans = 3,
                                                       detrend = FALSE,
                                                       demean = FALSE,
                                                       taper = 0.1,
                                                       power = 3,
                                                       n_groups = n_groups)))
tinytest::expect_equivalent(gain, rep(0.2, n_groups),
