...
points(value~x, resp_dl[resp_dl$term == "distributed_lag" & resp_dl$variable == "cumulative", ],
       type = "l", col = "red")

```

Distributed lag and earthtide
...
lag_dl <- recipe(wl~., kennel_2020) |>
  ...
  step_earthtide(datetime,
                 ...
                 catalog = catalog,
                 astro_update = 60) |>
  step_spline_n(datetime, df = 11, intercept = TRUE) |> 
  step_drop_columns(c(baro, datetime, et)) |>
  step_ols(wl~.) |> 
  prep() |>
  bake()
resp_dl <- lag_dl$get_response_data()
pred_dl <- lag_dl$get_predict_data()
